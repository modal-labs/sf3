...
async def test(n_samples: int, training_run_id: str = ""):
    ...
    checkpoint_volume = modal.Volume.from_name(checkpoint.checkpoints_volume_name)
    model_cls = Qwen3VLServer.with_options(  # pyright: ignore[reportAttributeAccessIssue]
        volumes={
            **cache_volumes,
            checkpoint.checkpoints_mount_path: checkpoint_volume,
        }
    )
    llm = model_cls(ckpt_path=checkpoint.path)
    llm.boot.remote()
...
async def main(n_samples: int = 100, training_run_id: str = ""):
    await test.remote.aio(n_samples, training_run_id)
